...
public class ConnectedSiteResourceTest
	...
	public void testDeleteAssetLibraryConnectedSite() throws Exception {
		super.testDeleteAssetLibraryConnectedSite();

		_testDeleteAssetLibraryConnectedSiteRemovesPairForStagedSite();
	}
	...
	private boolean _isConnected(
		...
		try {
			...
			if (Objects.equals(problem.getStatus(), "NOT_FOUND")) {
				return false;
			}
	...
	private void _testDeleteAssetLibraryConnectedSiteRemovesPairForStagedSite()
		...
		for (boolean disconnectViaLiveGroup : new boolean[] {true, false}) {
			...
			Group disconnectGroup = stagingGroup;

			if (disconnectViaLiveGroup) {
				disconnectGroup = liveGroup;
			}

			connectedSiteResource.deleteAssetLibraryConnectedSite(
				assetLibraryExternalReferenceCode,
				disconnectGroup.getExternalReferenceCode());
	...
	private void _testPutAssetLibraryConnectedSiteMirrorsSearchableToStagedSite()
		throws Exception {
		...
		Assert.assertFalse(liveConnectedSite.getSearchable());
		...
		Assert.assertFalse(stagingConnectedSite.getSearchable());
	}

	private void _testPutAssetLibraryConnectedSiteMirrorsStagedSite()
		throws Exception {
	...
	private void _testPutAssetLibraryConnectedSiteReturnsStagingType()
		...
		Group unstagedGroup = GroupTestUtil.addGroup();

		ConnectedSite unstagedConnectedSite =
			connectedSiteResource.putAssetLibraryConnectedSite(
				assetLibraryExternalReferenceCode,
				unstagedGroup.getExternalReferenceCode(), new ConnectedSite());

		Assert.assertEquals(
			ConnectedSite.StagingType.NONE,
			unstagedConnectedSite.getStagingType());
